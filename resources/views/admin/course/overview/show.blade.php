			...
			<div class="row">
				...
				<div class="col-md-4 mt-2">
					<select id="jumpToOptions" class="form-control rounded-0 text-dark">
						...
						@foreach($files as $f)
						<option {{ $f->id == $fileId ? 'selected' : '' }} data-link="/admin/course/{{ $course->id }}/overview/show/{{ $f->id }}">{{ $f->title }}</option>
						@endforeach
				...
				<div class="col-md-4 text-right">
					...
					@elseif ($next instanceof App\File)
						<span class="text-dark mr-3">NEXT ACTIVITY</span>
						<br>
						<a href="{{ route('course.overview.show.file', [$course->id, $next->id]) }}" class="btn btn-link" title="{{ $next->title }}"> {{ $next->title }}  ►</a>
					@endif
				</div>
